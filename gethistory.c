...
char *get_history_file(info_t *info)
{
        ...
        buf = malloc(sizeof(char) * (_strlen(dir) + _strlen(MY_SHELL_HISTORY_FILE) + 2));
        if (!buf)
                return NULL;
        ...
        _strcpy(buf, dir);
        _strcat(buf, "/");
        _strcat(buf, MY_SHELL_HISTORY_FILE);
...
int read_history(info_t *info)
{
        ...
        while (info->histcount-- >= MY_SHELL_HISTORY_MAX)
...
/**
 * renumber_history - Updates the line numbers in the command history linked list
 * @info: Structure containing potential arguments
 *
